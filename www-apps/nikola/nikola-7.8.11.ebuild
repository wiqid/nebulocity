...
SLOT="0"
KEYWORDS="~amd64"
IUSE="assets charts ghpages hyphenation jinja jupyter watchdog webmedia websocket yaml"
...
RDEPEND="${DEPEND}
	...
	>=dev-python/natsort-3.5.3[${PYTHON_USEDEP}]
	...
	ghpages? ( >=dev-python/ghp-import2-1.0.0[${PYTHON_USEDEP}] )
	hyphenation? ( >=dev-python/pyphen-0.9.1[${PYTHON_USEDEP}] )
	jinja? ( >=dev-python/jinja-2.7.2[${PYTHON_USEDEP}] )
	jupyter? ( >=dev-python/ipykernel-4.0.0[${PYTHON_USEDEP}]
...
src_install() {
	...
	unpack "docs/man/${PN}.1.gz" && doman "${PN}.1" # bug 619974
}
